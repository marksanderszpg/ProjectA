...
' IsExplorerBar ActiveX UserControl
' Copyright (c) 2004 by Fred.cpp
' http://www.Planet-Source-Code.com/vb/scripts/ShowCode.asp?txtCodeId=53572&lngWId=1
Option Explicit

Private Const strCurrentVersion As String = "1.70"
...
Event GroupClick(sGroup As Integer, bExpanded As Boolean)
...
Private Sub m_ScrollBar_GotFocus()
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    End With
    Set m_ScrollBar = UserControl.Controls.Add("VB.VScrollBar", "m_ScrollBar")
    m_ScrollBar.TabStop = False
    Set m_tempImg = UserControl.Controls.Add("VB.PictureBox", "m_tempImg")
    '    set m_ParentForm = usercontrol.p
    m_ItemTextColor = 0
    m_GroupTextColor = 0
    m_NotOnUse = 0
    m_ItemTextHoverColor = RGB(127, 127, 127)
    m_GroupHoverColor = RGB(127, 127, 127)
    UserControl.FontName = UserControl.Ambient.Font.Name
    UserControl.Extender.Align = 3
    m_ttIcon = TTIconInfo
    m_ttTitle = App.Title
    With PropBag
        'Properties comming soon
    End With
  ...
  '  Debug.Print Button
        ...
        If m_SpecialGroup.bExpanded Then
            For nj = 1 To m_SpecialGroup.iItemsCount
                ...
                If Y >= m_SpecialGroup.items(nj).mRect.Top And Y <= m_SpecialGroup.items(nj).mRect.bottom And m_SpecialGroup.items(nj).mRect.Left < x And m_SpecialGroup.items(nj).mRect.Right > x Then
                    ...
                    RedrawItem -1, nj, 2
                    'SetHandCur True
                    RaiseEvent ItemClick(m_SpecialGroup.Key, m_SpecialGroup.items(nj).Key)
                End If
            Next nj
        End If
    End If
    
    'Search ythe normal groups
    For ni = 1 To iGroups
        If Y >= cGroups(ni).mRect.Top And Y <= cGroups(ni).mRect.bottom And cGroups(ni).mRect.Left < x And cGroups(ni).mRect.Right > x Then
            'Cursor is over the group
        ...
        If cGroups(ni).bExpanded Then
            For nj = 1 To cGroups(ni).iItemsCount
                ...
                If Y >= cGroups(ni).items(nj).mRect.Top And Y <= cGroups(ni).items(nj).mRect.bottom And cGroups(ni).items(nj).mRect.Left < x And cGroups(ni).items(nj).mRect.Right > x Then
                    ...
                    RedrawItem ni, nj, 2
                    'SetHandCur True
                    RaiseEvent ItemClick(cGroups(ni).Key, cGroups(ni).items(nj).Key)
                End If
            Next nj
        End If
    Next ni
    ...
    If m_bDetailsGroup Then
        If Y >= m_DetailsGroup.mRect.Top And Y <= m_DetailsGroup.mRect.bottom And m_DetailsGroup.mRect.Left < x And m_DetailsGroup.mRect.Right > x Then
            ...
            UserControl.Refresh
            'SetHandCur True
            RaiseEvent GroupClick(-1, m_DetailsGroup.bExpanded)
        End If
    End If
    End If
...
Private Sub UserControl_Paint()
    ...
    If Not UserControl.Ambient.UserMode Then
        ...
        With m_cUxTheme
            ...
            .RightTextOffset = 0
            .Top = 48
            .Text = "http://mx.geocities.com/fred_cpp/isexplorerbar" ' Updates here
    ...
    Else
        ...
        'Get the theme name
        GetThemeName
        '<WIP desc="When the theme is Win classic and user changes the theme
        '           then control doesn't redraw." >
     '   Debug.Print "themename = " & sColorName
        With m_cUxTheme
            'Setup Some properties
            .hdc = UserControl.hdc
            .hwnd = UserControl.hwnd
...
Private Sub CalcRects()
    ...
    If m_bDetailsGroup Then
        ...
        If m_DetailsGroup.bExpanded Then
            ...
            On Error Resume Next
            If m_DetailsPicture = 0 Or m_DetailsPicture Is Nothing Then
                'There Isn't a Image
                m_DetailsGroup.lItemsHeight = CalcHeightRectText(20, UserControl.ScaleWidth - 32, m_DetailsGroupText) + CalcHeightRectText(20, UserControl.ScaleWidth - 32, m_DetailsGroupTittle) + 24
            ...
            Else
                ...
                m_DetailsGroup.lItemsHeight = CalcHeightRectText(20, UserControl.ScaleWidth - 48, m_DetailsGroupText) + CalcHeightRectText(20, UserControl.ScaleWidth - 48, m_DetailsGroupTittle) + 24
                'Set the Details Rect
                SetRect m_DetailsRect, 20, iTop + CalcHeightRectText(20, UserControl.ScaleWidth - 32, m_DetailsGroupTittle), UserControl.ScaleWidth - 32, iTop + m_DetailsGroup.lItemsHeight
                iTop = m_DetailsRect.bottom '+ 4
    ...
    Else
        ...
        If m_ScrollBar.Visible = False Then
...
Private Function GetGroupsByKey(ByVal sGroupKey As String) As Integer
    Dim ni As Integer
    For ni = 1 To iGroups
...
Private Function RedrawGroupHeader(iCurrentGroup As Integer)
    ...
    With cGroups(iCurrentGroup)
        ...
        m_cUxTheme.State = 1 '(Doesn't support some other states )
...
Private Function RedrawSpecialHeader()
    ...
    With m_SpecialGroup
        ...
        m_cUxTheme.Width = .mRect.Right - .mRect.Left
        m_cUxTheme.Height = .mRect.bottom - .mRect.Top
        m_cUxTheme.State = 1 '(Doesn't support other states )
...
Private Function RedrawDetailsHeader()
    ...
    With m_DetailsGroup
        ...
        m_cUxTheme.Width = .mRect.Right - .mRect.Left
        m_cUxTheme.Height = .mRect.bottom - .mRect.Top
        m_cUxTheme.State = 1 'iState (Doesn't support other states )
...
Private Sub RaiseError(sErrorDescription As String)
    MsgBox "An Error has ocurred!" & vbCrLf & _
            sErrorDescription, vbCritical, "isEcplorerBar"
End Sub
...
Public Sub AddItem(Group, sKey As String, sCaption As String, Optional iIcon As Integer)
    Dim iCurrentGroup As Integer
    If IsNumeric(Group) Then
        iCurrentGroup = Group
    Else
        'Get the index or key
        iCurrentGroup = GetGroupsByKey(Group)
    End If
    m_NotOnUse = 1
    If iCurrentGroup = -1 Then
        ...
        m_SpecialGroup.items(m_SpecialGroup.iItemsCount).Key = sKey
        m_SpecialGroup.items(m_SpecialGroup.iItemsCount).Caption = sCaption
        m_SpecialGroup.items(m_SpecialGroup.iItemsCount).sParent = "Special Group"
        m_SpecialGroup.items(m_SpecialGroup.iItemsCount).Index = m_SpecialGroup.iItemsCount
        m_SpecialGroup.items(m_SpecialGroup.iItemsCount).Icon = iIcon
    Else
        If iCurrentGroup = -3 Then
            RaiseWarning "Can't assign items to te Especified group"
            Exit Sub
        End If
      ...
      '  Debug.Print "group " & iCurrentGroup & " has " & cGroups(iCurrentGroup).iItemsCount & "Items"
        ...
        cGroups(iCurrentGroup).items(cGroups(iCurrentGroup).iItemsCount).Key = sKey
        cGroups(iCurrentGroup).items(cGroups(iCurrentGroup).iItemsCount).Caption = sCaption
        cGroups(iCurrentGroup).items(cGroups(iCurrentGroup).iItemsCount).sParent = Group
        cGroups(iCurrentGroup).items(cGroups(iCurrentGroup).iItemsCount).Index = cGroups(iCurrentGroup).iItemsCount
        cGroups(iCurrentGroup).items(cGroups(iCurrentGroup).iItemsCount).Icon = iIcon
    End If
    UserControl_Paint
    Exit Sub
noSuchGroup:
    RaiseWarning "The group '" & Group & "' desn't exist"
End Sub
...
' Desc: Show the special group
Public Sub ShowSpecialGroup()
    m_bSpecialGroup = True
    UserControl_Paint
    UserControl.Refresh
End Sub
...
Public Sub About() ' Authors information for this control
...
Public Sub ClearGroup(Group)
    ...
    Else
        'Get the index or key
        iGroupIndex = GetGroupsByKey(Group)
    End If
...
'       want to prevent redrawing using the
'       DisableUpdates method
Public Sub SetGroupChild(Group, pChild As PictureBox)
    Dim iGroupIndex As Integer
    If IsNumeric(Group) Then
        iGroupIndex = Group
    Else
...
'Desc: Remove all items in a group
'       if you will change lots of groups, you might
'       want to prevent redrawing using the
'       DisableUpdates method
Public Sub SetItemIcon(Group, item, iNewIcon As Integer, Optional bUpdate As Boolean = True)
    ...
    Else
        ...
        iGroupIndex = GetGroupsByKey(Group)
    End If
    'we got the index, now get the item index
    If IsNumeric(item) Then
        iItemIndex = item
    Else
        For nj = 1 To cGroups(iGroupIndex).iItemsCount
            If cGroups(iGroupIndex).items(nj).Key = item Then
                iItemIndex = nj
                Exit For
            End If
        Next
        'Item not found
        RaiseError "Item Key Especified doesn't match with the existing Items"
    End If
    ...
    If bUpdate Then UserControl_Paint
End Sub
